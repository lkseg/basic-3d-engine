...
inverse :: la.inverse
inverse_transpose :: la.inverse_transpose
...
vector2_to_vector4 :: proc "contextless" (v: $T/[2]$E, z := f32(0), w := f32(0)) -> Vector4 where IS_FLOAT(E) {
	return {v.x, v.y, z, w}
}
...
cast_rawptr_value :: #force_inline proc "contextless" (val: rawptr, $T: typeid) -> T {
	return T((cast(^T)val)^)
}
cast_uintptr_value :: #force_inline proc "contextless" (val: uintptr, $T: typeid) -> T {
	return T((cast(^T)rawptr(val))^)
}
rawptr_value :: proc{cast_rawptr_value, cast_uintptr_value}

cast_any_value :: #force_inline proc "contextless" (val: any, $T: typeid) -> T {
	return T((cast(^T)val.data)^)
}
cast_any :: #force_inline proc "contextless" (val: any, $T: typeid) -> T 
where relect.is_pointer(T) {
...
make_type_info_struct_iterator_from_any :: proc(a: any) -> (Type_Info_Struct_Iterator, bool) #optional_ok {
	info, ok := get_type_info_struct(a.id)
	if !ok {
		return {}, false
	}
	return Type_Info_Struct_Iterator{info = info, data= uintptr(a.data), index=0}, true
}
make_type_info_struct_iterator_from_info :: proc(data: rawptr, info: ^runtime.Type_Info_Struct) -> (Type_Info_Struct_Iterator, bool) #optional_ok {
	return Type_Info_Struct_Iterator{info = info, data= uintptr(data), index=0}, true
}
